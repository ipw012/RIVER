...
test_that("Basic functions work as expected", {
  ...
  FeatTrng = t(exprs(dataInput[,is.na(dataInput$N2pair)])) # training G data
  OutTrng = as.numeric(unlist(dataInput$Outlier[is.na(dataInput$N2pair)]))-1 # training E data
  ...
  expect_identical(rownames(FeatTest),c(sampleNames(dataInput[,!is.na(dataInput$N2pair)])
                         [seq(from=1,to=sum(!is.na(dataInput$N2pair)),by=2)],
                         sampleNames(dataInput[,!is.na(dataInput$N2pair)])
                         [seq(from=2,to=sum(!is.na(dataInput$N2pair)),by=2)]))
  expect_equal(length(unique(OutTest1)),2) # either outlier or nonoutlier
  expect_equal(length(unique(OutTest2)),2) # either outlier or nonoutlier
  ...
  logisticAllCV = glmnet::cv.glmnet(FeatAll, OutAll, lambda=costs,
                                family="binomial", alpha=0, nfolds=10)
  probFuncRv_Feat = getFuncRv_Feat(FeatAll, logisticAllCV$glmnet.fit, logisticAllCV$lambda.min)
  temp_val = predict(logisticAllCV$glmnet.fit, FeatAll,
                     s=logisticAllCV$lambda.min, type="response")

  ## getFuncRv_Feat works as expected
  expect_is(probFuncRv_Feat, "matrix")
  expect_equal(dim(probFuncRv_Feat)[1], dim(FeatAll)[1])
  expect_equal(probFuncRv_Feat, temp_val)

  posteriors = getFuncRvPosteriors(OutAll, probFuncRv_Feat, theta=theta_init)
  ...
  beta.cur = logistic.cur$beta[,which(logistic.cur$lambda == logisticAllCV$lambda.min)]
  temp_coeff = glmnet::glmnet(FeatAll, posteriors$posterior,
                              lambda=logisticAllCV$lambda.min, family="binomial", alpha = 0)
  expect_true(all.equal(as.vector(beta.cur), as.vector(temp_coeff$beta), tol = 1e-04))

  logisticModelAll <- integratedEM(FeatAll, OutAll, logisticAllCV$lambda.min,
