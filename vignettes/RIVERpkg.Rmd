...
```{r}
print(dataInput)
G = t(Biobase::exprs(dataInput)) # genomic features (G)
E = as.numeric(unlist(dataInput$Outlier))-1 # outlier status (E)
```
...
head(E)
```
`G` contains continuous values of genomic features (defined as $G$ in the objective function) while `E` contains binary values representing outlier status of same samples as `G` (defined as $E$ in the objective function).
...
```{r}
example_probs = data.frame(Subject=postprobs$indiv_name,Gene=postprobs$gene_name,RIVERpp=postprobs$RIVER_posterior,GAMpp=postprobs$GAM_posterior)
head(example_probs)
```
...
filename = system.file("extdata", "simulation_RIVER.gz", package = "RIVERpkg")
dataInput = getData(filename) # import experimental data
evaROC <- evaRIVER(dataInput, pseudoc=50, theta_init=matrix(c(.99, .01, .3, .7), nrow=2),costs=c(100, 10, 1, .1, .01, 1e-3, 1e-4), verbose=TRUE)
```
...
```{r}
filename = system.file("extdata", "simulation_RIVER.gz", package = "RIVERpkg")
system(paste('zcat ', filename, " | head  -2", sep=""), ignore.stderr=TRUE)
```
